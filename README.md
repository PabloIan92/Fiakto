...
- [x] ~~Filtrado por proximidad real (geohash)~~ — **decisión de Pablo (2026-08-14): no encarar.** El filtrado por localidad declarada (texto libre) ya alcanza; que sea el profesional quien juzgue si la distancia le conviene, no el sistema. En su lugar se construyó el chat interno (ver abajo).
...
- [x] **Chat por solicitud**: `GET`/`POST /api/requests/[id]/messages`, habilitado solo en los mismos estados donde ya existe "¿Algo salió mal?" (`accepted`/`in_progress`/`completed`/`closed` — antes de aceptar un presupuesto no hay ningún profesional específico con quien chatear). Componente `<RequestChat>` (polling cada 8s, reutiliza el mismo mecanismo de alertas — sonido/notificación — que `/cliente/solicitudes` y `/profesional/oportunidades`) embebido en el detalle de ambos roles.
- [x] **Ocultamiento automático de contacto** (`redactContactInfo`, `src/domain/messages.ts`): reemplaza emails y secuencias de 8+ dígitos (teléfonos, con o sin separadores) por `[contacto oculto]` antes de guardar el mensaje — best-effort (no detecta un número escrito en palabras), pero cubre el caso típico de pegar un WhatsApp o email. Deliberadamente no oclute montos en pesos (5-7 dígitos sin separador, como "150000") para no estorbar una conversación legítima de precio.
- [x] **Un admin puede leer cualquier chat** (mismo endpoint GET, `role === "admin"` pasa el chequeo) para moderar/auditar — no hay todavía una pantalla dedicada para eso, se lee vía la API o consultando Firestore directo.
- **Verificación**: cubierto con 14 tests (dominio + rutas + repositorio), todos en verde, `npm run build` OK. No se hizo una prueba en vivo con dos cuentas reales por falta de credenciales de las cuentas ya emparejadas (`test.fiakto.qa`, `ianvilona`) — recomendable que Pablo lo pruebe en `/profesional/oportunidades/[id]` (job `mgFNGNtGRgniEGhibjq3`, `completed`) y `/cliente/solicitudes/[id]` en staging.
...
- [ ] Mercado Pago: cobro, liberación y reembolso regulados (hoy es transferencia manual + comprobante).
- [ ] Órdenes de cambio (modificar un trabajo ya aceptado). Presupuesto/chat/evidencia ya están.
